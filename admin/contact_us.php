...
<body>
	...
	<section id="sidebar">
		<a href="dashboard.php" class="brand">
			<span class="text text2">Contact_us</span>
		</a>
		<?php
			...
			<div class="table-data">
				<div class="order">
					...
					<table>
						...
						<tbody>
						<?php
      $select_accounts = $conn->prepare("SELECT * FROM `contactus`");
      $select_accounts->execute();
      if($select_accounts->rowCount() > 0){
         while($fetch_accounts = $select_accounts->fetch(PDO::FETCH_ASSOC)){   
   ?>
    <tr>
      <th scope="row"><?= $fetch_accounts['contact_id']; ?></th>
      <td><?= $fetch_accounts['FullName']; ?></td>
	  <td><?= $fetch_accounts['email']; ?></td>
      <td><?= $fetch_accounts['message']; ?></td>
      <td><?= $fetch_accounts['contact_date']; ?></td>
	  <td><?= $fetch_accounts['status']; ?></td>
	
      <td><a href="admin.php?delete=<?= $fetch_accounts['contact_id']; ?>" 
